...
const CCMarker = (props) => {
  ...
  const damagedbuildingbyid = async (obj) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/get-id-data?id=${obj.t_array_id}`
      );
      ...
      console.error(
        "Error fetching building data for ID:",
        obj.t_array_id,
        error
      );
  ...
  const handleClick = async (ccpoint) => {
    ...
    if (ccpoint.t_array_id) {
      console.log(ccpoint.t_array_id)
      dispatch(buttonsActions.isloadingcc(true));
      console.log("1");
      ...
      if (fetchedData) {
        dispatch(buttonsActions.isloadingcc(false));
        console.log("2");
        const formattedEventData = {
          happend: fetchedData.happend || null,
          source: fetchedData.link
            ? decodeURIComponent(fetchedData.link)
            : null,
          // before: fetchedData.before || null,
          after: fetchedData.after || null,
          infotxt: fetchedData.infotxt
            ? decodeURIComponent(fetchedData.infotxt)
            : null,
        ...
        const updatedCcpoint = {
          ...ccpoint,
          event: ccpoint.event
            ? [...ccpoint.event, formattedEventData]
            : [formattedEventData],
        };
